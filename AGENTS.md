...
|---|---|---|---|
| Data Scraper Agent | JavaScript (Node.js, Puppeteer, `puppeteer-extra-plugin-stealth`) | Scrape Threads/X/Facebook text, enforce 3000px lazy-load scroll depth, and apply anti-bot evasion (proxy rotation + randomized user-agent). | **Input:** platform URLs, selectors, scraping config. **Output:** `raw_social_data.json` (`[{"text": "...", "timestamp": "...", "platform": "..."}]`). |
